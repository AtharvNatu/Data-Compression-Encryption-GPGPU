...
__constant int d_sbox_inverse[256] = 
{
...
void aes_ocl_add_round_key(byte_t state[AES_STATE_SIDE][AES_STATE_SIDE], __global byte_t *aes_round_key, byte_t round)
{
	// Code
    for (int i = 0; i < 4; i++)
    {
...
void aes_ocl_byte_sub_inverse(byte_t state[AES_STATE_SIDE][AES_STATE_SIDE])
{
    // Code
    for (int i = 0; i < 4; i++)
    {
...
void aes_ocl_shift_rows_inverse(byte_t state[AES_STATE_SIDE][AES_STATE_SIDE])
{
    // Code
...
void aes_ocl_mix_columns_inverse(byte_t state[AES_STATE_SIDE][AES_STATE_SIDE])
{
...
void ocl_inverse_cipher(__global byte_t input[AES_LENGTH], __global byte_t output[AES_LENGTH], __global byte_t* round_key)
{
    // Variable Declarations
    byte_t round = 0;
    ...
    {
        aes_ocl_shift_rows_inverse(state);
        aes_ocl_byte_sub_inverse(state);
        aes_ocl_add_round_key(state, round_key, round);
        aes_ocl_mix_columns_inverse(state); 
    }
...
__kernel void aes_ocl_ecb_decrypt(__global byte_t* d_ciphertext, __global byte_t* d_plaintext, __global byte_t* d_round_key, int data_size)
{
    // Code
    ...
    if (id < data_size)
    {
        ocl_inverse_cipher(d_ciphertext + id * 16, d_plaintext + id * 16, d_round_key);
    }  
}
