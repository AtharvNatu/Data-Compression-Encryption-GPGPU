...
void CLFW::ocl_create_kernel(const char* ocl_kernel_name, const char* ocl_kernel_arg_types, ...)
{
    ...
    for (auto i = 0; ocl_kernel_arg_types[i] != '\0'; i++)
    {
        switch(ocl_kernel_arg_types[i])
        {
            ...
            case 'l':
                clfwTypes->char_data = va_arg(kernel_args_list, cl_ulong);
                ocl_exec_status(clSetKernelArg(ocl_kernel, i, sizeof(cl_ulong), (void*)&clfwTypes->ulong_data));
            break;
...
float CLFW::ocl_execute_kernel(size_t ocl_global_work_size, size_t ocl_local_work_size)
{
    // Code
    ...
    float ocl_gpu_time = sdkGetTimerValue(&ocl_timer);  
...
size_t CLFW::get_global_work_size(int local_size, unsigned int global_size)
{
    // Code
...
void CLFW::host_release_mem(unsigned char **host_ptr)
{
    // Code
    if (*host_ptr)
    {
